    ...
    try:
        if state == 'WAIT_SITE_USER':
            ...
            if state == 'ADM_WAIT_WIN_RATE':
                ...
                conn.close()
                context.user_data.clear()
                users = get_db().execute("SELECT telegram_id FROM users WHERE is_banned = 0").fetchall()
                success, failed = 0, 0
                await update.message.reply_text("⏳ جاري بدء الإذاعة العامة...")
                for u in users:
                    try:
                ...
                try:
                    ...
                    await update.message.reply_text(f"✅ تم إرسال الرسالة إلى العميل <code>{t_id}</code> بنجاح.", parse_mode="HTML")
                except Exception as e:
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == 'WAIT_WIN_SHOT':
        ...
        await update.message.reply_text("✅ تم إرسال صورة الإصابة إلى الإدارة بنجاح.")
        await show_main_menu(update, context)
        return
        ...
        await send_all_admins(
            context,
            f"💬 <b>رسالة دعم مع صورة من عميل!</b>\n"
            f"• العميل: <code>{user_id}</code>\n"
            f"• النص: {html.escape(caption)}",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💬 الرد على العميل", callback_data=f"reply_support_{user_id}")]])
        )
        conn = get_db()
        admins = conn.execute("SELECT telegram_id FROM users WHERE is_admin = 1").fetchall()
        ...
        await update.message.reply_text("✅ تم إرسال صورتك ورسالتك إلى فريق الدعم بنجاح.")
        await show_main_menu(update, context)
        return
